...
$marketUpdateObject = json_decode($_POST['marketUpdateObject']);
$nego = 0;//흥정여부

$title = $marketUpdateObject -> title;
$contents = $marketUpdateObject -> contents;
$price = $marketUpdateObject -> price;
$category = $marketUpdateObject -> category;
$isNego = $marketUpdateObject -> isNego;
if($isNego){
    $nego = 1;
}else{
    $nego = 0;
...
$sql = "UPDATE market_post SET title = '$title', contents = '$contents', price = '$price', category = '$category', isNego = '$nego' WHERE idx = '$_GET[postNumber]'";
...
if($imgList_client == "[]"){
    ...
    //이미지 테이블 insert
     for($i = 0; $i < count($imgList); $i++){
        echo $imgList[i];
        $sql_img = "INSERT INTO market_img (postNumber,fileName) VALUES ('$_GET[postNumber]','$imgList[$i]')";
        echo $sql_img;
    ...
    for($i = 0; $i < count($removeList); $i++){
        echo $removeList[$i];
        $sql_removeImg = "DELETE FROM market_img WHERE fileName = '$removeList[$i]'";
        echo $sql_removeImg;
